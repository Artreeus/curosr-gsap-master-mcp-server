    ...
    try {
        switch (name) {
            case 'understand_and_create_animation': {
                ...
                const context = args?.context || 'react';
...
Just describe what you want in natural language and I'll generate production-ready code!`
            ...
            case 'get_gsap_api_expert': {
                const apiElement = args?.api_element;
                ...
                if (GSAP_COMPLETE_API.CORE_METHODS[api]) {
                    const method = GSAP_COMPLETE_API.CORE_METHODS[api];
                    ...
                    if ('performance_tips' in method && method.performance_tips) {
                        ...
                        method.performance_tips.forEach((tip) => {
                            result += `- ${tip}\n`;
                        });
                    }
                }
                else if (GSAP_COMPLETE_API.PLUGINS[apiElement] || GSAP_COMPLETE_API.PLUGINS[apiElement.replace('Plugin', '')]) {
                    const pluginKey = GSAP_COMPLETE_API.PLUGINS[apiElement] ? apiElement : apiElement.replace('Plugin', '');
                    const plugin = GSAP_COMPLETE_API.PLUGINS[pluginKey];
                    ...
                    if ('performance_optimization' in plugin && plugin.performance_optimization) {
                        ...
                        plugin.performance_optimization.forEach((tip) => {
                            result += `- ${tip}\n`;
                        });
                    }
                }
                    ...
                    const suggestions = allApis.filter(api => api.toLowerCase().includes(apiElement.toLowerCase()) ||
            ...
            case 'generate_complete_setup': {
                ...
                const plugins = args?.plugins || ['ScrollTrigger', 'SplitText'];
                const performance_level = args?.performance_level || 'optimized';
            ...
            case 'debug_animation_issue': {
                ...
                const expected_behavior = args?.expected_behavior;
                if (!issue) {
            ...
            case 'optimize_for_performance': {
                const animationCode = args?.animation_code;
                const target = args?.target || '60fps-desktop';
                if (!animationCode) {
                ...
                const tips = targetTips[target] || targetTips['60fps-desktop'];
                tips.forEach((tip) => {
                    result += `- ${tip}\n`;
                });
            ...
            case 'create_production_pattern': {
                ...
                const industry = args?.industry || 'portfolio';
                if (!patternType) {
...
<main class="main-content">
  <!-- Your main content -->
                ...
                result += `\`\`\`html\n${htmlStructures[patternType] || '<!-- HTML structure for ' + patternType + ' -->'}\n\`\`\`\n\n`;
                result += `## 💅 Required CSS\n\n`;
                ...
                const customizations = industryCustomizations[industry] || industryCustomizations.portfolio;
